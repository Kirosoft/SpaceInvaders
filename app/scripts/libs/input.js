...
export default class Input {
  ...
  initMouse() {
    ...
    this.canvas.addEventListener('mousewheel', mouseWheelBound, false);
    this.canvas.addEventListener('DOMMouseScroll',
      mouseWheelBound, false);
  ...
  mousemove(event) {
    var internalWidth = parseInt(this.canvas.offsetWidth) ||
      ig.system.realWidth;
    var scale = ig.system.scale * (internalWidth / ig.system.realWidth);

    var pos = {left: 0, top: 0};
    if (this.canvas.getBoundingClientRect) {
      pos = this.canvas.getBoundingClientRect();
    }
  ...
  bindTouch(selector, action) {
    ...
    element.addEventListener('touchstart',
      function(ev) {that.touchStart(ev, action);}, false);
    element.addEventListener('touchend',
      function(ev) {that.touchEnd(ev, action);}, false);
